...
function createRobotBody( part, length, material ) {
    ...
    var cylinderBottom = new THREE.Mesh(
        new THREE.CylinderGeometry( 50, 12, length/2, 18 ), material );
    cylinderBottom.position.y = length/4;
    part.add( cylinderBottom );
    totalHeight += length/4;

    var cylinderTop = new THREE.Mesh(
        new THREE.CylinderGeometry( 12, 50, length/2, 18 ), material );
    cylinderTop.position.y = 3*length/4;
    part.add( cylinderTop );
    totalHeight += length/4;
    ...
    sphere.position.y = length;
    part.add( sphere );
    totalHeight += 40; // Diameter of the sphere

    // Adjust arm position
    arm.position.y = totalHeight;
}
