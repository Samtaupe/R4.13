...
function render() {
	...
	var azimuth = effectController.azimuth * Math.PI / 180.0;
	var altitude = effectController.altitude * Math.PI / 180.0;

	light.position.x = Math.sin(azimuth) * Math.cos(altitude);
	light.position.y = Math.sin(altitude);
	light.position.z = Math.cos(azimuth) * Math.cos(altitude);
